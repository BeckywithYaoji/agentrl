"""Qwen3-1.7B LoRA-GRPO stability and LoRA-only checkpoint/resume smoke."""
import argparse
import hashlib
import json
import re
...
ARTIFACT = Path('artifacts/milestone7c3')
CHECKPOINT = Path('/root/autodl-tmp/checkpoints/agentrl_m7c3/resume_ckpt')
CHECKPOINT_ROOT = Path('/root/autodl-tmp/checkpoints/agentrl_m7c3')
...
def write_checkpoint_metadata(path, metadata):
    ...
    metadata_path = path / 'agentrl_m7c3_metadata.json'
    metadata_path.write_text(json.dumps(metadata, indent=2, sort_keys=True), encoding='utf-8')
    return metadata_path


def read_checkpoint_metadata(path):
    return json.loads((path / 'agentrl_m7c3_metadata.json').read_text(encoding='utf-8'))
...
def summarize_memory(step_records, samples):
    ...
    summary['leak_assessment'] = 'no clear progressive leak in 5-step smoke'
...
def run_phase(directory, *, phase, total_steps, save_checkpoint=False, resume_checkpoint=None, restored_global_step=None,
    ...
    with open_dict(config):
        ...
        config.trainer.project_name = 'agentrl-m7c3'
    ...
    config_record = dict(event='config', phase=phase, base_model=MODEL_PATH, base_path=base_path,
                         ...
                         sample_id=SAMPLE_ID, reward='R0 exact-match', retriever='train-only TinyBM25',
                         rollout_n=4, temperature=1.0, top_p=1.0, max_response_length=2048,
                         per_turn_generation_cap=256, lr=1e-6, total_phase_steps=total_steps,
    ...
    class SmokeTrainer(PPOTrainerSync):
        ...
        def _update_actor(self, batch, metrics):
            ...
            group = {'phase': phase, 'global_step': self.global_steps, 'sample_id': SAMPLE_ID,
            ...
            record = {'phase': phase, 'global_step': self.global_steps, 'sample_id': SAMPLE_ID,
                      ...
                      'actor_loss': actor_metrics.get('actor/pg_loss') or actor_metrics.get('actor/ppo_loss'),
    ...
    try:
        ...
        if save_checkpoint:
            ...
            trainer.actor_rollout_wg.save_checkpoint(str(CHECKPOINT), global_step=total_steps, max_ckpt_to_keep=None)
            trainer.actor_rollout_wg.save_lora_training_state(str(CHECKPOINT), total_steps)
        checkpoint_metadata = None
        if save_checkpoint:
            checkpoint_metadata = {'global_step': total_steps, 'trainable_params': a.get('trainable_numel'),
                                   'optimizer_owned_params': a.get('optimizer_numel'),
                                   'optimizer_frozen_numel': a.get('optimizer_frozen_numel'),
                                   'optimizer_state_entries': a.get('optimizer_state_entries'),
                                   'scheduler_state': a.get('lr_scheduler_state'),
                                   'saved_lora_fingerprints': a.get('lora'),
                                   'saved_base_fingerprints': a.get('base'),
                                   'sample_id': SAMPLE_ID, 'start_adapter': str(ADAPTER.resolve()),
                                   'checkpoint_path': str(CHECKPOINT), 'save_lora_only': True}
            write_checkpoint_metadata(CHECKPOINT, checkpoint_metadata)
        ...
        if a['optimizer_steps'] < total_steps or not lora_changed or base_changed:
...
def run_train_save(directory, args):
    return run_phase(directory, phase='train-save', total_steps=5, save_checkpoint=True,
                     gpu_memory_utilization=args.gpu_memory_utilization)


def run_resume_step(directory, args):
    metadata = read_checkpoint_metadata(CHECKPOINT)
    ok, message = validate_checkpoint_metadata(metadata, expected_global_step=5)
    if not ok:
        raise RuntimeError(message)
    report = run_phase(directory, phase='resume-step', total_steps=1, resume_checkpoint=CHECKPOINT,
...
def main():
    ...
    parser.add_argument('--mode', choices=['train-save', 'resume-step'], required=True)
    ...
    if args.mode == 'train-save' and directory.exists() and args.overwrite:
        shutil.rmtree(directory)
    directory.mkdir(parents=True, exist_ok=True)
    if args.mode == 'train-save':
