"""Formal SFT-initialized veRL GRPO-R2 with val-only selection."""
import hashlib
import json
import os
import shutil
...
def expand_per_sample(value, count):
    def scalar(item):
        ...
        for i in range(count):
            ...
            for key, column in value.items():
                if isinstance(column, dict):
                    row[key] = expand_per_sample(column, count)[i]
                    continue
                try:
                    row[key] = scalar(column[i]) if len(column) == count and not isinstance(column, (str, bytes)) else scalar(column)
                except Exception:
                    row[key] = scalar(column)
            rows.append(row)
        return rows
    try:
        if len(value) == count and not isinstance(value, (str, bytes)):
            try:
                return [scalar(value[i]) for i in range(count)]
            except Exception:
                return [scalar(item) for item in value]
    except Exception:
        pass
    return [scalar(value) for _ in range(count)]
...
def run(directory, *, checkpoint_root, reload_from=None):
    ...
    from agentrl.reward import parse_trajectory, ACTION_RE, normalized_em, compute_reward
    ...
    with open_dict(config):
        ...
        config.trainer.project_name = 'agentrl-m6e2'
        config.trainer.experiment_name = 'sft-grpo-r2-100steps'
        ...
        config.reward.custom_reward_function.path = str(Path('src/agentrl/verl_reward_adapter.py').resolve())
    ...
    emit(trace, dict(event='config', reward='R2', model=snapshot, n=4, steps=100, lr=1e-6,
    ...
    class FormalTrainer(PPOTrainerSync):
        ...
        def _update_actor(self, batch, metrics):
            fields = tq.kv_batch_get(keys=batch.keys, partition_id=batch.partition_id, select_fields=None)
            ...
            diagnostics = [parse_trajectory(text) for text in decoded]
            if len(rewards) != 4: raise RuntimeError(f'expected n=4, got {len(rewards)}')
            extra_infos = expand_per_sample(fields.get('extra_info', {}), len(rewards)) if 'extra_info' in fields.keys() else []
            reward_models = expand_per_sample(fields.get('reward_model', {}), len(rewards)) if 'reward_model' in fields.keys() else []
            if self.global_steps == 1:
                emit(trace, dict(event='update_fields', fields=sorted(str(k) for k in fields.keys())))
            component_rows = []
            for value, text, parsed, extra, reward_model in zip(rewards, decoded, diagnostics, extra_infos, reward_models, strict=False):
                extra = extra if isinstance(extra, dict) else {}
                reward_model = reward_model if isinstance(reward_model, dict) else {}
                details = compute_reward(
                    question=extra.get('question', ''),
                    ground_truth=reward_model.get('ground_truth', ''),
                    raw_trajectory=text,
                    requires_search=extra.get('requires_search'),
                )
                answer_em = float(details['answer_em'])
                protocol_valid = bool(details['protocol_valid'])
                decision_correct = details.get('decision_correct')
                excess_searches = int(details.get('excess_searches', 0))
                component_rows.append(dict(
                    sample_id=extra.get('sample_id'), index=extra.get('index'),
                    requires_search=extra.get('requires_search'), ground_truth=reward_model.get('ground_truth'),
                    final_r2=float(value), recomputed_r2=float(details['reward']['r2']),
                    answer_correct=bool(answer_em), answer_em=answer_em,
                    answer_f1=details.get('answer_f1'), protocol_valid=protocol_valid,
                    first_action=details.get('first_action', parsed.first_action),
                    decision_correct=decision_correct, search_count=int(details.get('search_count', parsed.search_count)),
                    excess_searches=excess_searches,
                    components=dict(answer_credit=1.0 if answer_em else 0.0,
                                    protocol_penalty=(-0.2 if answer_em and not protocol_valid else (-0.1 if not answer_em and not protocol_valid else 0.0)),
                                    decision_penalty=(-0.1 if answer_em and decision_correct == 0.0 else 0.0),
                                    oversearch_penalty=(-min(0.10, 0.02 * excess_searches) if answer_em else 0.0))))
            group = dict(global_step=self.global_steps, rewards=rewards, r2=rewards, mean=mean(rewards),
                         variance=pvariance(rewards), all_zero=all(x == 0 for x in rewards),
                         all_perfect=all(x == 1 for x in rewards),
                         zero_variance=pvariance(rewards) == 0,
                         search_count=[row['search_count'] for row in component_rows] if component_rows else [p.search_count for p in diagnostics],
                         protocol_valid=[row['protocol_valid'] for row in component_rows] if component_rows else [p.protocol_valid for p in diagnostics],
                         answer_correct=[row['answer_correct'] for row in component_rows],
                         decision_correct=[row['decision_correct'] for row in component_rows],
                         component_breakdown=component_rows,
                         sample_id=[row.get('sample_id') for row in component_rows],
                         requires_search=[row.get('requires_search') for row in component_rows],
        ...
        def _validate(self):
            ...
            parsed = [parse_trajectory(row['output']) for row in records]
            r2_scores = [float(row['score']) for row in records]
            ...
            for record, parsed_row in zip(records, parsed, strict=True):
                ...
                answer_scores.append(normalized_em(parsed_row.answer, source['answer'], source.get('aliases', ())))
                decisions.append(float(bool(parsed_row.search_count) == bool(source['metadata']['requires_search'])))
            em = mean(answer_scores)
            ...
            record = dict(event='selection_val', global_step=self.global_steps,
                          ...
                          r2_mean=mean(r2_scores), r2_scores=r2_scores, answer_scores=answer_scores)
            if self.best_key is None or key > self.best_key:
                old = self.best_path
                root = Path(checkpoint_root)
                best_dir = root / 'best'
                if old and Path(old).exists(): shutil.rmtree(old)
                self._save_checkpoint()
                new = root / f'global_step_{self.global_steps}'
                if not (new / 'actor').is_dir(): raise RuntimeError('best checkpoint missing actor')
                if best_dir.exists() or best_dir.is_symlink():
                    if best_dir.is_dir() and not best_dir.is_symlink(): shutil.rmtree(best_dir)
                    else: best_dir.unlink()
                shutil.move(str(new), str(best_dir))
                latest = root / 'latest'
                if latest.exists() or latest.is_symlink():
                    if latest.is_dir() and not latest.is_symlink(): shutil.rmtree(latest)
                    else: latest.unlink()
                latest.symlink_to('best')
                self.best_key, self.best_path = key, str(best_dir)
    ...
    try:
        ...
        report = dict(decision='PASS' if steps == 100 and changed and trainer.best_path else 'BLOCKED',
            ...
            r2_mean=mean(g['mean'] for g in groups),
            reward_hacking_flags=dict(repeated_search_groups=sum(any(c > 1 for c in g['search_count']) for g in groups),
                                      high_search_groups=sum(any(c >= 3 for c in g['search_count']) for g in groups),
                                      long_response_groups=sum(max(g['response_lengths']) > 1800 for g in groups),
                                      wrong_high_r2=sum(any((row.get('answer_em') == 0.0 and row.get('final_r2', 0.0) >= 0.5) for row in g.get('component_breakdown', [])) for g in groups)))
...
def main():
    ...
    directory = Path(args.artifact_dir) if args.artifact_dir else Path('artifacts/milestone6e2') / datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S')
    directory.mkdir(parents=True, exist_ok=True)
    os.environ['AGENTRL_M6E2_ARTIFACT'] = str(directory.resolve())
    try:
